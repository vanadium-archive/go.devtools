...
type xbuilder func(*tool.Context, *Manager, profiles.Target) (*profiles.Target, error)
...
func (m *Manager) Install(ctx *tool.Context, target profiles.Target) error {
	...
	if target.CrossCompiling() {
		...
		// for cgo to work.
		builder := xcompilers[xspec{runtime.GOARCH, runtime.GOOS}][xspec{target.Arch, target.OS}]
		if builder != nil {
			xtarget, err := builder(ctx, m, target)
			if err != nil {
				return err
			}
			//		merge env...
...
func linux_to_linux(ctx *tool.Context, m *Manager, target profiles.Target) (t *profiles.Target, e error) {
	...
	installNgFn := func() error {
		xgccSrcDir := filepath.Join(m.root, "profiles", "csrc", "crosstool-ng-1.20.0")
		...
		if err := profiles.RunCommand(ctx, "./configure", []string{fmt.Sprintf("--prefix=%v", xgccOutDir)}, nil); err != nil {
			return err
		}
	...
	if err := profiles.AtomicAction(ctx, installNgFn, xgccOutDir, "Build and install crosstool-ng"); err != nil {
		return nil, err
	}
	...
	xgccToolDir := filepath.Join(xgccOutDir, targetABI)
	installXgccFn := func() error {
		...
		dataPath, err := project.DataDirPath(ctx, "devtools")
		if err != nil {
			return err
		}
	...
	if err := profiles.AtomicAction(ctx, installXgccFn, xgccToolDir, "Build arm/linux gcc tools"); err != nil {
		ctx.Run().RemoveAll(xgccToolDir)
		return nil, err
	}

	// Create arm/linux gcc symlinks.
	xgccLinkDir := filepath.Join(xgccOutDir, "xarm")
	installLinksFn := func() error {
		if err := ctx.Run().MkdirAll(xgccLinkDir, profiles.DefaultDirPerm); err != nil {
			return err
		}
	...
	if err := profiles.AtomicAction(ctx, installLinksFn, xgccLinkDir, "Create arm/linux gcc symlinks"); err != nil {
		return nil, err
	}
	// Fill in target.
...
func darwin_to_linux(ctx *tool.Context, m *Manager, target profiles.Target) (*profiles.Target, error) {

...
func init() {
	tool.InitializeRunFlags(&profileDriver.Flags)
}
...
var (
	...
	profileFlag, buildTagFlag string
	rootDir                   string
)
...
func init() {
	...
	manifest := filepath.Join(rootDir, ".jiri-profiles")
...
func runEnv(env *cmdline.Env, args []string) error {
	...
	if target == nil {
		return fmt.Errorf("Target %q is not installed for profile %q", buildTagFlag, profileFlag)
	}
...
func runUpdate(env *cmdline.Env, args []string) error {
	...
	applyCommand(args, env, ctx, targetFlag,
		func(mgr profiles.Manager, ctx *tool.Context, target profiles.Target) error {
			...
			if err == profiles.ErrNoIncrementalUpdate {

...
// github.com.
func vanadiumGitHubMirror(ctx *util.Context, testName string, _ ...TestOpt) (_ *TestResult, e error) {
...
func sync(ctx *util.Context, mirror Mirror, projects string) (*xunit.TestSuite, error) {
...
func clone(ctx *util.Context, mirror Mirror, projects string) (error) {
	dirname := filepath.Join(projects, mirror.name)
	return ctx.Git().Clone(mirror.googlesource, dirname)
}

func pull(ctx *util.Context, mirror Mirror, projects string) (error) {
	dirname := filepath.Join(projects, mirror.name)
	opts := util.RootDirOpt(dirname)
	return ctx.Git(opts).Pull("origin", "master")
}

func push(ctx *util.Context, mirror Mirror, projects string) (error) {
	dirname := filepath.Join(projects, mirror.name)
	opts := util.RootDirOpt(dirname)

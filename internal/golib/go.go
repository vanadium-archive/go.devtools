...
func PrepareGo(ctx *tool.Context, env map[string]string, args []string, extraLDFlags string) ([]string, error) {
	switch args[0] {
	case "build", "install":
		...
		// will override this.
		var err error
		args, err = setBuildInfoFlags(ctx, args, env, extraLDFlags)
		if err != nil {
			return nil, err
		}
		fallthrough
...
func setBuildInfoFlags(ctx *tool.Context, args []string, env map[string]string, extraLDFlags string) ([]string, error) {
